...
def run_inference(args):
    ...
    o3d.io.write_point_cloud(os.path.join(args.output_dir, "fused.ply"), pcd_down)

    total_time = time.time() - start_time
    print(
        f"Point cloud completed + saved in {total_time:.2f} seconds."
    )
    ...
    # Save mesh
    o3d.io.write_triangle_mesh(os.path.join(args.output_dir, "fused_mesh.ply"), mesh_crop)
    total_time = time.time() - start_time
    print(
        f"Mesh completed + saved in {total_time:.2f} seconds."
    )
